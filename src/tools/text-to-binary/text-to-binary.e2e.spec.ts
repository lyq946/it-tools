...
test.describe('Tool - Text to ASCII binary', () => {
  ...
  test('Has correct title', async ({ page }) => {
    await expect(page).toHaveTitle('Text to ASCII binary - Dev Tools Hub');
  });

  test('Text to binary conversion', async ({ page }) => {
    await page.getByTestId('text-to-binary-input').fill('dev-tools-hub');
  ...
  test('Binary to text conversion', async ({ page }) => {
    ...
    expect(text).toEqual('dev-tools-hub');
  });
});
